...
static void vox_el_init_poller(void) {
  ...
  *(uint64_t*)(ev + 4) = (uint64_t)(uint32_t)vox_el_efd;
  if (epoll_ctl(vox_el_epfd, VOX_EPOLL_CTL_ADD, vox_el_efd, (struct epoll_event*)ev) != 0) {
  ...
  if (kevent(vox_el_kq, (const struct kevent*)ev, 1, NULL, 0, NULL) != 0) {
...
void vox_impl_el_poller_wake(void) {
  ...
  kevent(vox_el_kq, (const struct kevent*)ev, 1, NULL, 0, NULL);
#elif defined(_WIN32)
...
void vox_impl_el_poller_wait(int32_t timeout_ms) {
...
#if defined(__linux__)
  char ev[12];
  int n = epoll_wait(vox_el_epfd, (struct epoll_event*)ev, 1, timeout_ms);
  ...
  char ev[VOX_KEVENT_SZ];
  kevent(vox_el_kq, NULL, 0, (struct kevent*)ev, 1, &ts);
#elif defined(_WIN32)
