...
4. stdlib 基座：`std/sync` 已从 `i32` MVP 提升为 concrete `i32/i64` 句柄族（`MutexI32/AtomicI32/MutexI64/AtomicI64`）；`std/io` 网络最小 TCP API（`net_connect/net_send/net_recv/net_close`）在解释器与 C 后端可用。
