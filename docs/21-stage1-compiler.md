...
- Stage0/Stage1/Stage2 当前仍使用 `String.slice(start, end) -> String` 作为子串基础操作；切片方向通过标准库 `std/string::StrView`（`owner + lo/hi`）补齐。Stage2 已支持 `&T` / `&'static T` 语法，但当前在类型层仍按 `T` 过渡语义处理。
