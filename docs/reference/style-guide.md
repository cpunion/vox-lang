...
`vox_rt_*` runtime 能力的目标是“清零语言内建暴露”，统一下沉到标准库实现（`std` + `ffi` + 平台封装）。
...
1. 新增跨边界 I/O 接口默认使用 `rawptr + len`，避免把 C-string 语义扩散到上层 API。
2. `String` 仅用于必须按 C-string 交互的兼容接口（例如部分路径/命令 API）。
3. 需要 NUL 终止时由适配层显式构造，不允许“隐式依赖输入自然带 `\\0`”。
