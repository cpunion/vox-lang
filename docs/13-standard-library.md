...
- `std::sync`：并发原语（`Mutex[T]/Atomic[T]` 泛型 API 已落地；并保留 concrete 兼容封装）
...
- `std::sync` 当前主 API 为泛型句柄：`Mutex[T]` / `Atomic[T]`（当前 `T` 由 `SyncScalar` 约束，已覆盖 `i32/i64`）；底层统一基于 runtime `i64` handle intrinsic。`fetch_add/swap/load/store` 已在解释器与 C 后端对齐。
- 同时保留兼容封装：`MutexI32/AtomicI32` 与 `MutexI64/AtomicI64`，用于平滑迁移现有代码。
