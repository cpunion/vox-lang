...
- 非内置宏调用采用“内联优先 + 调用糖回退”的过渡语义：
  - 先尝试模板内联：
    - 本模块：`name!(...)` 对应到本模块函数 `fn name(...)`，且函数体是单条 `return <expr>;`（当前不含 block/match/try block 等局部绑定模板），则将 `<expr>` 克隆到调用点并按形参名替换实参。
    - 泛型模板支持显式实参替换：`name[T, 3]!(...)` 会在内联时同步替换模板中的类型参数与常量参数（缺少显式泛型实参时回退）。
    - 跨模块（`pkg.name!(...)`）：仅在模板对跨模块内联安全时启用（当前要求模板不依赖目标模块的额外局部符号），否则回退。
