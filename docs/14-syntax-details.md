...
- 变更方法（`Vec.push`/`Vec.insert`/`Vec.clear`/`Vec.extend`/`Vec.pop`/`Vec.remove`）：receiver 必须是 **place**（可写位置）。
  - `Vec.push`/`Vec.insert`/`Vec.clear`/`Vec.extend`/`Vec.pop`/`Vec.remove` 要求 receiver 的根绑定是 `let mut`（不可变绑定会报错）。
  - Stage0/Stage1：支持局部变量（如 `v.push(x)`、`v.insert(i, x)`、`v.clear()`、`v.extend(w)`、`v.pop()`、`v.remove(i)`），以及可变局部 struct 的直接字段（如 `s.items.push(x)`、`s.items.insert(i, x)`、`s.items.clear()`、`s.items.extend(w)`、`s.items.pop()`、`s.items.remove(i)`）。
  - Stage2：额外支持可变局部 struct 的多级字段（如 `o.inner.items.push(x)`、`o.inner.items.insert(i, x)`、`o.inner.items.clear()`、`o.inner.items.extend(w)`、`o.inner.items.pop()`、`o.inner.items.remove(i)`）。
