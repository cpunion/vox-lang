...
- 初始化表达式必须是 **const expression**（常量表达式），目前仅支持：
  ...
  - `match`（当前 const 子集支持 `_`、bind、`true/false`、整数字面量、字符串字面量、enum **unit** pattern（无 payload））
- 整数运算语义与运行时保持一致：
  - `+ - *`、`& | ^`、`<< >>` 按目标整数位宽执行（wrapping）。
  - `/ %` 在除数为 `0` 时编译期报错。
...
- 不支持在 const 初始化中调用函数、构造 struct/enum payload（当前仅支持 enum unit 变体简写，例如 `const X: E = .A`）。
