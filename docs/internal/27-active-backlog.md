...
- [ ] A44 字符串 FFI/运行时收敛：从 C-string 语义迁移到 `ptr + len`，并优先平台 API
  ...
  - [x] A44-1b `std/io::NetConn.try_send` 已改为 `std/sys::send(handle, ptr, len)`，平台分支直接走 OS `send`（`@build`），避免向 `c_runtime` 扩增网络桥接符号。
  ...
  - [ ] A44-2c `std/sys` 增加 `open_read(path)` 接口：linux/darwin/wasm 已接通 `open(O_RDONLY)`；Windows 当前为分架构状态（amd64 走 `_sopen_dispatch`，x86 仍是 bootstrap 兼容占位）。
    ...
    - Blocked (2026-02-22): locked bootstrap (`v0.2.22`) does not support FFI `&mut` params, while CRT alternatives on windows-x86 are either varargs (`_open/_sopen`) or require `FILE*` typed signatures that currently conflict with `rawptr` import declarations. Resume after bootstrap lock includes FFI `&mut` and/or a safer pointer-typed FFI surface.
