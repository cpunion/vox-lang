...
- [ ] A44 字符串 FFI/运行时收敛：从 C-string 语义迁移到 `ptr + len`，并优先平台 API
  ...
  - [x] A44-2c `std/sys` 增加 `open_read(path)` 接口：linux/darwin/wasm 走 `open(O_RDONLY)`；Windows 当前通过 `vox_sys_open_read`（C runtime 内部调用 CRT `_open(path, 0, 0)`，规避 varargs 原型冲突）。
