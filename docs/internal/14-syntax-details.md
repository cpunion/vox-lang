...
- 整数字面量（当前）：
  - 当上下文期望 `u64` 时，十进制字面量允许完整 `0..18446744073709551615`。
  - 当上下文期望 `usize` 时，范围由目标指针位宽决定：
    - 32 位目标：`0..4294967295`
    - 64 位目标：`0..18446744073709551615`
...
- `lo/hi` 支持十进制整数字面量（允许前缀 `-`）和 `char` 字面量（如 `'a'`、`'\n'`）。
...
- `a.b(...)`（调用上下文）
  - 若 `a`（或 `a.b.c` 的根）是当前作用域中的局部变量/参数：尝试解析为**值方法调用**。
    ...
    - 对实现了 trait/impl 方法集的类型，进入 trait/impl 调度（含 associated type）。
    - 无可用方法时报错。
...
- 标准库内部采用分层约定：`__*` 仅由 `std/runtime` 使用，`std` 其它模块通过 `std/runtime` 封装访问，以降低 intrinsic 变更对滚动发布的影响。
- 用户代码应通过 `std/fs`、`std/process`、`std/io`、`std/sync` 等封装接口使用，不直接接触 `__*`。
...
- `@same_layout(A, B) -> bool`
  - 兼容保留，当前实现会给出弃用告警，建议使用 `@bitcastable(A, B)`。
