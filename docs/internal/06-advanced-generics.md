...
- 偏序不比较方法体语义；
- `where comptime` 已支持一阶不等式蕴含：
  - 同一 lhs、同一 rhs 基底比较（如 `@size_of(T) <= 8` 强于 `@size_of(T) <= 16`）；
  - 基于 rhs 符号链的传递推理（如 `@size_of(T) <= @align_of(U)` 且 `@align_of(U) <= 16`，可推出 `@size_of(T) <= 16`）。
- 仍未覆盖更复杂蕴含：多分支布尔组合约束联立推理、`!=` 的完整域推理等。
