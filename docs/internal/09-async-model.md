...
4. async 入口与测试可运行（最小执行器，v0）：
   ...
   - 取消分支支持可选 frame 重绑定钩子：优先 hint 形态 `cancel_drop_hint_with(rt, cx, hint, f)` / `cancel_drop_hint(cx, hint, f)`，再回退 `cancel_drop_state_with(rt, cx, state, f)`、`cancel_drop_with(rt, cx, f)`、`cancel_drop_state(cx, state, f)`、`cancel_drop(cx, f)`（用于宿主按 Future/frame 类型 + state/spins/token 做更细粒度释放或重置）。
   - 取消资源清理支持可选钩子：优先 hint 形态 `cancel_cleanup_hint_with(rt, cx, hint)` / `cancel_cleanup_hint(cx, hint)`，再回退 `cancel_cleanup_state_with(rt, cx, state)`、`cancel_cleanup_with(rt, cx)`、`cancel_cleanup_state(cx, state)`、`cancel_cleanup(cx)`（若存在则在取消返回前执行）。
   - 取消结果传播支持可选钩子：
     - 优先 hint 形态 `cancel_return_hint_with(rt, cx, hint)` / `cancel_return_hint(cx, hint)`，再回退 `cancel_return_with(rt, cx)`、`cancel_return(cx)`；
...
fn cancel_hint(c: Context, state: i32, spins: i32) -> CancelHint; // 可选：构建细粒度取消提示
fn cancel_drop_hint_with[R, F](rt: R, c: Context, hint: CancelHint, f: F) -> F; // 可选钩子
fn cancel_drop_state_with[R, F](rt: R, c: Context, state: i32, f: F) -> F; // 可选钩子
fn cancel_drop_with[R, F](rt: R, c: Context, f: F) -> F; // 可选钩子
fn cancel_drop_hint[F](c: Context, hint: CancelHint, f: F) -> F; // 可选钩子
fn cancel_drop_state[F](c: Context, state: i32, f: F) -> F; // 可选钩子
fn cancel_drop[F](c: Context, f: F) -> F; // 可选钩子
fn cancel_cleanup_hint_with[R](rt: R, c: Context, hint: CancelHint) -> (); // 可选钩子
fn cancel_cleanup_state_with[R](rt: R, c: Context, state: i32) -> (); // 可选钩子
fn cancel_cleanup_with[R](rt: R, c: Context) -> (); // 可选钩子
fn cancel_cleanup_hint(c: Context, hint: CancelHint) -> (); // 可选钩子
fn cancel_cleanup_state(c: Context, state: i32) -> (); // 可选钩子
fn cancel_cleanup(c: Context) -> (); // 可选钩子
fn cancel_return_hint_with[R, T](rt: R, c: Context, hint: CancelHint) -> T; // 可选钩子
fn cancel_return_with[R, T](rt: R, c: Context) -> T; // 可选钩子
fn cancel_return_hint[T](c: Context, hint: CancelHint) -> T; // 可选钩子
...
1. runtime/executor 体验继续增强（当前已支持 `default_runtime + *_with` 注入，默认 runtime 基于 wake-token 事件等待，Linux/macOS/*BSD/Windows 已分别接入 `epoll+kqueue+IOCP` 的底层等待路径，并保留 wasm/其他目标回退分支；`EventSource + ReadyQueue + drain_ready_once` 继续作为统一接口层）。
