...
- `Stage1`：进入冻结维护期，不再进入默认测试/发布链路；仅作为历史 bootstrap 参考代码保留。
- `Stage2`：作为唯一的语言能力演进主线（类型系统、泛型/trait 增强、标准库扩展、工具链能力前移等都在此推进）。

冻结并不意味着不测试：默认门禁聚焦 `Stage0`（Go 单测）与 `Stage2`（rolling 自举 + 自测套件）。

## 自举收敛原则（避免无限 StageN 与长期退化）

- 编译器阶段固定为 `Stage0 -> Stage1 -> Stage2`。`Stage3` 仅指工具链，不再新增编译器阶段编号。
- `Stage1` 采用“完全冻结”策略：不再作为日常自举链路的一部分。
...
- `make test-active`：冻结维护期主线门禁（`stage0` 单测 + `stage2(locked) -> stage2(new)` rolling 引导链路 + `stage2` 自测套件）。
...
> 注：`stage1_integration_test` 为历史回归集；当前默认门禁与发布链路已切换为 stage2 rolling 自举。
...
- 可通过 `make test-stage2-selfhost`（rolling 引导构建）与 `make test-stage2-tests`（stage2 测试套件）单独执行（不依赖 stage1）。
