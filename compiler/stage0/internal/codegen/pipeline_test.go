...
func TestPipelineStringOrderedComparisonCompilesAndRuns(t *testing.T) {
	cc, err := exec.LookPath("cc")
	if err != nil {
		t.Skip("cc not found")
	}
...
func TestPipelineNegativeISizeLiteralFoldsToConst(t *testing.T) {
	cc, err := exec.LookPath("cc")
	if err != nil {
		t.Skip("cc not found")
	}
  ...
  return x as i32;
	...
	if !strings.Contains(csrc, "INT64_C(-5)") {
		t.Fatalf("expected folded negative literal, got C:\n%s", csrc)
	}
...
func TestPipelineCastU64MaxLiteralInCBackend(t *testing.T) {
	cc, err := exec.LookPath("cc")
	if err != nil {
		t.Skip("cc not found")
	}
