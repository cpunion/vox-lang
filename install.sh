...
DEFAULT_VERSION="${VOX_INSTALL_DEFAULT_VERSION:-v0.2.11}"
...
Usage:
  ...
  curl -fsSL https://raw.githubusercontent.com/cpunion/vox-lang/main/install.sh | bash -s -- --version v0.2.11

Options:
  --local                 Build from local vox-lang repo (rolling selfhost) then install.
  --download              Force release-binary install.
  --version <tag>         Release tag, e.g. v0.2.11 (default: latest).
  --platform <os-arch>    e.g. darwin-arm64, linux-amd64, windows-amd64.
  --repo <owner/repo>     GitHub repo (default: cpunion/vox-lang).
  --default-version <tag> Fallback tag when latest query fails (default: v0.2.11).
